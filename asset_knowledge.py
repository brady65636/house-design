...
PAINT_FAMILY_GUIDANCE: dict[str, dict[str, str | list[str]]] = {
    "warm_white": {
...
FLOOR_GUIDANCE: dict[str, dict[str, str | list[str]]] = {
    "light_fine_grain": {
        "description": "浅色、细密、低色差的木纹；作为地面背景安静而明亮。",
    ...
    "light_natural_oak": {
        "description": "自然浅橡木色，保留温和木纹而不偏黄，是平衡明亮与温度的主地面。",
...
def _card_for_paint(asset: dict[str, Any]) -> dict[str, Any]:
    guidance = PAINT_FAMILY_GUIDANCE.get(asset.get("family_id", ""), {})
    tone = _tone_text(str(asset.get("tone", "")))
    finish = _finish_text(str(asset.get("finish", "")))
    return {
        "visual_description": f"{guidance.get('description', asset.get('name_zh', '墙漆'))}{tone}，{finish}。",
        ...
        "style_roles": ["大面积墙面背景", "以明暗和底色建立空间基调"],
    }
...
def _card_for_wallpaper(asset: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "avoid_when": "避免放在被门窗切碎的墙面；避免与强石纹、高色差木纹或另一种强图案并置。",
        "style_roles": ["完整墙面的焦点或细腻背景", "提供图案、节奏或近看微纹理"],
    }


def _card_for_guided_surface(asset: dict[str, Any], guidance_map: dict[str, dict[str, str | list[str]]]) -> dict[str, Any]:
    guidance = guidance_map.get(asset.get("family", ""), {})
    return {
        ...
        "style_roles": ["地面或顶面的主要材料语言", "组织空间连续性与视觉重量"],
    }
...
def build_asset_knowledge(
    ...
    *,
    ...
    index = {
        ...
        "assets": [
            {"id": card["id"], "category": card["category"], "brief": card["brief"]}
            for card in cards
        ],
    }
    card_library = {
