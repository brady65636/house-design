...
墙纸库已完成 16 个原创系列。`viewer/app/data/wallpaperCatalog.json` 是 Blender、Three.js、生成脚本、交互面板和测试共用的唯一目录数据源。
...
同一款墙纸可能同时铺在东、西、南、北向墙面。若基础色完全乘以方向光，朝向太阳的墙会被洗成近白，背光墙则会压成深棕，看起来像换了产品。运行时因此把墙纸基础色拆成两部分：

- `78%` 作为与墙面朝向无关的印刷色基准；
- `22%` 保留方向光响应，用于读取墙角、体块与轻微受光变化。

Normal、Roughness、sheen、HDR 反射和真实尺度 UV 仍按物理材质计算；基准只约束 Base Color 的跨朝向漂移。实现位于 `WALLPAPER_APPEARANCE_CALIBRATION`，并统一作用于目录中的全部 16 款墙纸。按需释放贴图时必须同时清空 `map` 与复用同一基础色纹理的 `emissiveMap`，避免保留已释放的纹理引用。
...
- 16 个稳定 Asset ID、16 个视觉系列、16 套来源记录；
- 七款循环基础色四边平均像素误差为 0；
- 32 张 PBR 母版的尺寸、存在性与 Blender 节点自动检查；
- 16 个 Blender 墙纸材质都包含 Base Color、Normal、Roughness、Height；
- 客厅西墙 UV 范围为约 `0–4.51m × 0–2.80m`；
- Blender/GLB 独立验证 27 项全部通过；
- Three.js 正式构建、目录测试和运行时逐款切换必须通过。
- 同一墙纸应用到不同朝向墙面时必须保持同一印刷底色，只允许存在克制的自然受光差异；不得出现像换了色号的整体明暗漂移。
